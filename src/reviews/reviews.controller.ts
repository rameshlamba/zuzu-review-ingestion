...
export class ReviewsController {
  ...
  async getPlatforms(): Promise<{ platforms: string[] }> {
    try {
      ...
      throw new HttpException('Failed to fetch platforms', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async getHotelReviews(
    ...
    try {
      ...
      if (result.pagination.total === 0) {
        throw new HttpException(`No reviews found for hotel ID ${hotelId}`, HttpStatus.NOT_FOUND);
      }
      ...
      throw new HttpException('Failed to fetch hotel reviews', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async getReviewById(@Param('id', ParseIntPipe) id: number): Promise<ReviewResponseDto> {
    try {
      ...
      throw new HttpException('Failed to fetch review by ID', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
}
